...
// ================== Filtro PRAP Administrativo — Desde BD ==================
(function () {
  "use strict";
  ...
  if (!selDireccion || !inpResp || !btnBuscar || !btnLimpiar || !tbody || !lblResumen) return;
  ...
  async function resolveYearId() {
    try {
      ...
      if (!found) return 1;
      YEAR_ID = parseInt(found.id, 10) || 1;
      return YEAR_ID;
    } catch {
      YEAR_ID = 1;
      return 1;
  ...
  async function cargarDatos() {
    ...
    try {
      await resolveYearId();
      const resp = await fetch(`php/filtro_prap_admin_api.php?anio=${encodeURIComponent(YEAR_ID)}`);
      const data = await resp.json();
      if (!data.ok || !Array.isArray(data.rows)) throw new Error();
      TODOS = data.rows;
      const seen = new Set();
      DIRS = [];
      TODOS.forEach((r) => {
  ...
  function ejecutarFiltro() {
    const dirSel = selDireccion.value;
    const respQ = norm(inpResp.value);
    const filtrados = TODOS.filter((r) => {
      if (dirSel && String(r.dir_id) !== dirSel) return false;
      if (respQ && !norm(r.resp || "").includes(respQ)) return false;
      return true;
    });
  ...
  function renderTabla(items) {
    tbody.innerHTML = "";
    if (!items.length) {
      tbody.innerHTML = `<tr><td colspan="8" style="text-align:center">No hay resultados.</td></tr>`;
      lblResumen.textContent = "Mostrando 0 resultados";
      return;
    }
    items.forEach((it) => {
      ...
      a.href = `prap.html?id=${encodeURIComponent(it.dir_id)}&year=${YEAR}`;
      a.textContent = "Abrir";
      tdVer.appendChild(a);
    });
