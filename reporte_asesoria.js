(() => {
  ...
  // Para mostrar solo una variante de setiembre/septiembre
  const MES_CANON = (m) => {
    const s = norm(m);
  ...
  // ---------- Estado ----------
  let YEAR     = getYear();
  let MR       = {};   // { meses, tipoVisita, cicloEscolar, asesoria, circuito01..05 }
  let VISITAS  = [];   // visitas_jefatura_v1[YEAR]
  ...
  function renderKpis(filtered){
    ...
    for (const r of filtered){
      ...
      else if (r.tipoVisita) { // categoría fuera de catálogo
    ...
    // --- Mes
    // mostramos los 12 con 0 si aplica
    const mesesCanon = [
      "Enero","Febrero","Marzo","Abril","Mayo","Junio",
  ...
  function bindUI(){
    ...
    // Reaccionar a cambios de año
    window.addEventListener("yearchange", (ev)=>{
      YEAR = String(ev?.detail?.year ?? getYear());
      loadMR(); loadVisitas(); fillAsesoriaSelect(); applyFilter();
    });

    // Reaccionar a aportes desde el formulario/jefatura
    window.addEventListener("storage", (ev)=>{
      if (ev.key === SINK_KEY || ev.key === MR_KEY(YEAR) || ev.key === 'visitas_jefatura_last_update'){
        loadMR(); loadVisitas(); fillAsesoriaSelect(); applyFilter();
      }
    });
  }
